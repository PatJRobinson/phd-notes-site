# Interviews and Methodology Evidence and Provenance

## Purpose

Separate what this workflow currently has as evidence from what remains design scaffold or future empirical work.

## Current evidence

The strand currently has four evidence types:

- methodological design notes explaining the interpretive, cross-layer approach;
- interview design artefacts translating concepts into participant-facing prompts;
- coding and analysis scaffolds for extracting assumption records and developing themes;
- public literature notes grounding reflexive thematic analysis, work-as-done, distributed cognition, infrastructure, and situated action.

## Public grounding now reflected in the compiled site

- [Braun and Clarke](/01_literature/hci-infrastructure-practice/braun-clarke-reflexive-thematic-analysis): supports reflexive, abductive thematic analysis with theory as sensitising concepts.
- [Hollnagel](/01_literature/safety-resilience-theory/hollnagel-resilience-engineering-work-as-done): supports practice/work-as-done as a safety and resilience site.
- [Hutchins](/01_literature/hci-infrastructure-practice/hutchins-distributed-cognition): supports analysing understanding across people, artefacts, representations, environments, and time.
- [Star and Ruhleder](/01_literature/hci-infrastructure-practice/star-ruhleder-infrastructure): supports infrastructure, installed base, invisible work, and breakdown as ways assumptions become visible.
- [Suchman](/01_literature/hci-infrastructure-practice/suchman-situated-action): supports the limit of formal representations and plans.

## Internal evidence

The strongest internal material is the alignment between the methodology justification, interview guide, concept-to-question map, and coding scheme. Together they show that the empirical design is coherent.

## Missing evidence

- Participant data.
- Pilot-interview feedback.
- Analytic memos showing movement from excerpts to assumption records to themes.
- A worked example of one assumption record from an interview excerpt.
- Final decision on participant categories and unit of analysis.

## Status

Green for study design coherence. Amber for public scholarly grounding because the literature notes still need exact bibliographic details. Red for empirical findings because no participant evidence is compiled yet.

## Source provenance

## Public sources

- [Literature map](/01_literature/literature-map)
- [Braun and Clarke literature note](/01_literature/hci-infrastructure-practice/braun-clarke-reflexive-thematic-analysis)
- [Hollnagel literature note](/01_literature/safety-resilience-theory/hollnagel-resilience-engineering-work-as-done)
- [Hutchins literature note](/01_literature/hci-infrastructure-practice/hutchins-distributed-cognition)
- [Star and Ruhleder literature note](/01_literature/hci-infrastructure-practice/star-ruhleder-infrastructure)
- [Suchman literature note](/01_literature/hci-infrastructure-practice/suchman-situated-action)

## Internal provenance

<!-- AUTONOTES:PRIVATE-SOURCES:START -->
- [methodological-justification.md](../../../04_research_methodologies/methodological-justification.md)
- [note-on-analytical-approach.md](../../../04_research_methodologies/note-on-analytical-approach.md)
- [Core Concepts.md](../../../02_my_notes/Interview%20Preperation/Core%20Concepts.md)
- [Candidate Questions and Themes.md](../../../02_my_notes/Interview%20Preperation/Candidate%20Questions%20and%20Themes.md)
- [Semi-Structured Interview Guide.md](../../../02_my_notes/Interview%20Preperation/Semi-Structured%20Interview%20Guide.md)
- [Coding Scheme.md](../../../02_my_notes/Interview%20Preperation/Coding%20Scheme.md)
<!-- AUTONOTES:PRIVATE-SOURCES:END -->
