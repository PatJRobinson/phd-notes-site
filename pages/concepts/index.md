# Concepts Index

## Core thesis concepts

- [Integration](integration.md)
- [System behaviour](system-behaviour.md)
- [System boundary](system-boundary.md)
- [Interface](interface.md)
- [Configuration](configuration.md)
- [Deployment context](deployment-context.md)
- [Modularity](modularity.md)
- [ROS 2-style robotics](ros2-style-robotics.md)

## RQ-specific concepts

- [Integrator](integrator.md)
- [Working understanding](working-understanding.md)
- [Partial observability](partial-observability.md)
- [Local knowledge](local-knowledge.md)
- [Debugging](debugging.md)
- [Assurance claim](assurance-claim.md)
- [Continuous assurance](continuous-assurance.md)

## Rotifer concepts

- [Rotifer as assumption tracing](rotifer-as-assumption-tracing.md)
- [Assumption-bearing invariant](assumption-bearing-invariant.md)
- [Visibility and observability](visibility-and-observability.md)

## Safety/security concepts

- [Safety/security relation](safety-security-relation.md)
- [Command authority](command-authority.md)
- [Integrator responsibility boundary](integrator-responsibility-boundary.md)
- [SROS2 policy semantics](sros2-policy-semantics.md)

## Methodology concepts

- [Working understanding](working-understanding.md)
- [Partial observability](partial-observability.md)
- [Local knowledge](local-knowledge.md)

## Historical/recoverable concepts

- Assumption misalignment: currently historical but still useful, represented through [Assumption-bearing invariant](assumption-bearing-invariant.md) and [Recoverable fragments](../workflows/recoverable-fragments/index.md).
- ROSness: promoted as valid for the [ROS 2-style robotics](ros2-style-robotics.md) thread, with older security-invariant provenance preserved.
- Human as overseer: recoverable for RQ4/future synthesis.
